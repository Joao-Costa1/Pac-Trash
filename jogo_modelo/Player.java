...
public class Player {
    ...
    private Image papel; // criando uma variavel do tipo "Image" que irá receber o icone do lixo;
    ...
    // metodos GET e SET para cada atributo privado
    ...
    public Player(){
        ...
        posicaoPlayer(); // chama metodo de posicionar o jogador;

    }

    public void posicaoPlayer() {

        personagem = vDown.getImage(); // personagem pegando a imagem do obejeto "pInicial";
        height     = personagem.getHeight(null); // atribuindo a altura do personagem;
        width      = personagem.getWidth(null); // atribuindo a largura do personagem;
        // definindo a posição inicial do personagem no meio da tela;
    ...
    // Criando objetos que recebem as imagens de cada objeto do jogo;
    ImageIcon vLeft       =  new ImageIcon(getClass().getResource("/Imagens/lixitoE.png"));
    ImageIcon vRight      =  new ImageIcon(getClass().getResource("/Imagens/lixitoD.png"));
    ImageIcon vDown       =  new ImageIcon(getClass().getResource("/Imagens/lixitoC.png"));
    ImageIcon vUp         =  new ImageIcon(getClass().getResource("/Imagens/lixito.png"));
    ImageIcon bolaDePapel =  new ImageIcon(getClass().getResource("/Imagens/boladepapel.png"));
    ImageIcon garrafa     =  new ImageIcon(getClass().getResource("/Imagens/garrafa.png"));
    ImageIcon lixo        =  new ImageIcon(getClass().getResource("/Imagens/lixo.png"));
    ImageIcon wall        =  new ImageIcon(getClass().getResource("/Imagens/parede.png"));
    ImageIcon portal1     =  new ImageIcon(getClass().getResource("/Imagens/portal.png"));
    ImageIcon R1          =  new ImageIcon(getClass().getResource("/Imagens/r1.png"));
    ImageIcon R2          =  new ImageIcon(getClass().getResource("/Imagens/r2.png"));
    ImageIcon R3          =  new ImageIcon(getClass().getResource("/Imagens/r3.png"));
    ImageIcon I_Banana    =  new ImageIcon(getClass().getResource("/Imagens/banana.png"));
    ImageIcon space       =  new ImageIcon(getClass().getResource("/Imagens/pause.png"));
    ImageIcon over        =  new ImageIcon(getClass().getResource("/Imagens/game_over.png"));
    ImageIcon win         =  new ImageIcon(getClass().getResource("/Imagens/vitoria.png"));
    ...
    // metodo responável por gerar as imagens que serão carregadas na tela;
    public void load() {
    ...
    public void update() { //metodo resposável por atualizar a posição do personagem na tela;

        // variáveis de posição x e y, recebendo a direção x e y que o personagem está se movimentando de acordo com o valor da variavel "sentido";
        if(start) {
        switch(sentido) {
        case 0: // quando aperta o espaço
            ...
            dy=0; // atribuindo valor na variavel de direção responsável pelo eixo 'y' (- = cima);
        ...
        case 8:
            ...
            if(mp[mpy - 1][mpx] != 18) { // verificando se a posição seguinte da matriz na "linha-apy" é diferente de 18 (parede), para o personagem se movimentar;
                personagem = vUp.getImage(); // mudando a imagem da direção do personagem para cima;
                width  = personagem.getWidth(null);
                height = personagem.getHeight(null);
                dx=0; // zerando o valor da variável de eixo 'x', para que o personagem não se mova nos dois eixos ao mesmo tempo;
                dy=-20; // atribuindo valor na variavel de direção responsável pelo eixo 'y' (- = cima);
                mpy-=1; // movimenta a matriz;
                py = py + dy; // eixo y do personagem recebendo o valor de dx para "movimenatr/mudar" a posição do personagem;
        ...
        case 2: // caso sentido seja "2" mover para baixo
            ...
            if(mp[mpy + 1][mpx] != 18) { // verificando se a posição seguinte da matriz na "linha+apy" é diferente de 18 (parede), para o personagem se movimentar;
                ...
                height = personagem.getHeight(null);
                dx=0; // zerando o valor da variável de eixo 'x', para que o personagem não se mova nos dois eixos ao mesmo tempo;
                dy=20; // atribuindo valor na variavel de direção responsável pelo eixo 'y' (+ = baixo);
                mpy+=1; // movimenta a matriz;
        ...
        case 6: // caso sentido seja "6" mover para direita
            ...
            if(mp[mpy][mpx + 1] != 18) { // verificando se a posição seguinte da matriz na "coluna+apx" é diferente de 18 (parede), para o personagem se movimentar;
                // verificando se o personagem entrou no portal da direita e posicionando eel no portal da esquerda;

                personagem = vRight.getImage(); // mudando a imagem da direção do personagem para direita;
                width  = personagem.getWidth(null);
                height = personagem.getHeight(null);

                dx=27; // atribuindo valor na variavel de direção responsável pelo eixo 'x' (+ = direita);
                dy=0; // zerando o valor da variável de eixo 'y', para que o personagem não se mova nos dois eixos ao mesmo tempo;
                mpx+=1; // movimenta a matriz;

                px = px + dx; // eixo x do personagem recebendo o valor de dx para "movimenatr/mudar" a posição do personagem;
        ...
        case 4: // caso sentido seja "4" mover para esquerda
            ...
            if(mp[mpy][mpx - 1] != 18) { // verificando se a posição seguinte da matriz na "coluna-apx" é diferente de 18 (parede), para o personagem se movimentar;
                // verificando se o personagem entrou no portal da esquerda e posicionando eel no portal da direita;

                personagem = vLeft.getImage(); // mudando a imagem da direção do personagem para esquerda;
                width  = personagem.getWidth(null);
                height = personagem.getHeight(null);

                dx=-27; // atribuindo valor na variavel de direção responsável pelo eixo 'x' (- = esquerda);
                dy=0; // zerando o valor da variável de eixo 'y', para que o personagem não se mova nos dois eixos ao mesmo tempo;
                mpx-=1; // movimenta a matriz;
        ...
        if(start) {
            switch(mp[getMpy()][getMpx()]) { 
                case 0: // papel 2 + pontos
                    this.score = score + 2;
                    mp[getMpy()][getMpx()] = 8; // muda o numero na matriz, para se possivel indentificar qual era o numero anterior (8=0; 80=5; 81=6; 82=7; 83=10; 84=20; 85=30);
                break;
    ...
    // metodo responsável por atribuir os valores iniciais da matirz e o score;
    ...
    public void keyPressed(KeyEvent e) { // metodo que identifica um "evento" de entrada do teclado, no caso do "keyPressed" ele informa qual tecla o usuário esta pressionando;
    
        int cod = e.getKeyCode(); // variável responsável por receber o código da recla pressionada pelo usuário;
        ...
        if(start == true && cod == KeyEvent.VK_UP && mp[mpy][mpx] !=18) { // verificando se o código da tecla pressionada é o evento da "seta para cima" e se a posição da matriz não é igual a 18 (parede);
        ...
        if(start == true && cod == KeyEvent.VK_DOWN && mp[mpy][mpx] !=18) { // verificando se o código da tecla pressionada é o evento da "seta para baixo" e se a posição da matriz não é igual a 18 (parede);
        ...
        if(start == true && cod == KeyEvent.VK_LEFT && mp[mpy][mpx] !=18) { // verificando se o código da tecla pressionada é o evento da "seta para esquerda" e se a posição da matriz não é igual a 18 (parede);

            sentido = 4; // esquerda

        }

        if(start == true && cod == KeyEvent.VK_RIGHT && mp[mpy][mpx] !=18) { // verificando se o código da tecla pressionada é o evento da "seta para direita" e se a posição da matriz não é igual a 18 (parede);

            sentido = 6; // direita

        }

        if(jogo == false && cod == KeyEvent.VK_ENTER) { // se jogo for false quer dizer que o player ganhou ou que ele predeu o jogo;
            
            // quando aperta "ENTER" as variáveis que indicam se o usuário perdeu ou não o jogo recebem false e o jogo recebe true, desse maneira o jogo "recomeça";
            G_win = false;
            G_over = false;
            jogo = true;
        }
    ...
    public void keyReleased(KeyEvent e) {
        ...
        // quando o usuário soltar qualquer tecla, o personagem para de andar;
